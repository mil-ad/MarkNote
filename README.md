...
- **Editor preview works under Anki's editor CSP.** The editor page only allows scripts from Anki's own paths and `/_addons/`, so the preview loads the renderer from the addon folder (via `setWebExports`) and reads field content through the `anki/NoteEditor` JavaScript API rather than probing the DOM.
